...
# Define the Inference Function
def Inference_1(Encoder, Decoder, dataloader):
    with torch.no_grad():
        Encoder.eval()
        Decoder.eval()
        for batch_idx, (source, target) in enumerate(dataloader):
...
def Progression(Encoder, Decoder):
    pass
...
# Load the JSON File 
with open('./OneDrive_1_11-11-2023/progression.json', 'r') as f:
    test_json = json.load(f)

# Replace keys in each entry
modified_json_entries = []

for entry in test_json:
    modified_entry = {}
    
    for key, value in entry.items():
        modified_key = 'num0' if key == 'year' else 'str0' if key == 'race' else 'num1' if key == 'Time' else 'num3' if key == 'Distance' else key
        modified_entry[modified_key] = value

    # Append the modified entry to the list
    modified_json_entries.append(modified_entry)

# Convert each JSON to string
modified_json_strings = []

for entry in modified_json_entries:
    modified_json_strings.append(json.dumps(entry))

# Convert each string to indices
modified_json_indices = []

for entry in modified_json_strings:
    modified_json_indices.append(source_sentence_to_index(entry))

# Pad the indices
modified_json_indices = [line + [source_vocab['<pad>']] * (500 - len(line)) for line in modified_json_indices]

# Convert to numpy array
modified_json_indices = np.array(modified_json_indices)

# Convert to Pytorch Tensor
modified_json_indices = torch.from_numpy(modified_json_indices)

# Find the Length of the Dataset
print(len(modified_json_indices))

# Create the Dataset
modified_json_dataset = CustomDataset(modified_json_indices, test_targets[:len(modified_json_indices)])

# Create the Dataloader
modified_json_dataloader = DataLoader(modified_json_dataset, batch_size=32, shuffle=True)

# Define the Inference Function
Inference(encoder, decoder, modified_json_dataloader)
